...
def get_boxes3d_pred(boxes1, boxes2):
    ...
    if num1 >= num2:
        for i in range(num1):
            num = 0
            for j in range(num2):
                iou_min = max(boxes1[i, 0], boxes2[j, 0])
                iou_max = min(boxes1[i, 2], boxes2[j, 2])
                highth = min(boxes1[i, 2] - boxes1[i, 0], boxes2[j, 2] - boxes2[j, 0])
                if (iou_max - iou_min) / highth > 0.70:
                    bexes_3d.append([boxes1[i, 1], boxes2[j, 1], (boxes1[i, 0] + boxes2[j, 0]) / 2,
                                     boxes1[i, 3], boxes2[j, 3], (boxes1[i, 2] + boxes2[j, 2]) / 2])
                    num += 1
            if num == 0:
                boxes_ner = []
                for j in range(num2):
                    iou_min = max(boxes1[i, 0], boxes2[j, 0])
                    iou_max = min(boxes1[i, 2], boxes2[j, 2])
                    highth = boxes1[i, 2] - boxes1[i, 0]
                    if (iou_max - iou_min) / highth > 0.1:
                        boxes_ner.append([boxes1[i, 1], boxes2[j, 1], (boxes1[i, 0] + boxes2[j, 0]) / 2,
                                          boxes1[i, 3], boxes2[j, 3], (boxes1[i, 2] + boxes2[j, 2]) / 2])
                boxes_ner = np.array(boxes_ner)

                bexes_3d.append([boxes_ner[0, 0], boxes_ner[:, 1].sum() / boxes_ner.shape[0],
                                 boxes_ner[:, 2].sum() / boxes_ner.shape[0],
                                 boxes_ner[0, 3], boxes_ner[:, 4].sum() / boxes_ner.shape[0],
                                 boxes_ner[:, 5].sum() / boxes_ner.shape[0]])

    else:
        for i in range(num2):
            num = 0
            for j in range(num1):
                iou_min = max(boxes1[j, 0], boxes2[i, 0])
                iou_max = min(boxes1[j, 2], boxes2[i, 2])
                highth = min(boxes1[j, 2] - boxes1[j, 0], boxes2[i, 2] - boxes2[i, 0])
                if (iou_max - iou_min) / highth > 0.70:
                    bexes_3d.append([boxes1[j, 1], boxes2[i, 1], (boxes1[j, 0] + boxes2[i, 0]) / 2,
                                     boxes1[j, 3], boxes2[i, 3], (boxes1[j, 2] + boxes2[i, 2]) / 2])
                    num += 1
            if num == 0:
                boxes_ner = []
                for j in range(num1):
                    iou_min = max(boxes1[j, 0], boxes2[i, 0])
                    iou_max = min(boxes1[j, 2], boxes2[i, 2])
                    highth = boxes2[i, 2] - boxes2[i, 0]
                    if (iou_max - iou_min) / highth > 0.1:
                        boxes_ner.append([boxes1[j, 1], boxes2[i, 1], (boxes1[j, 0] + boxes2[i, 0]) / 2,
                                          boxes1[j, 3], boxes2[i, 3], (boxes1[j, 2] + boxes2[i, 2]) / 2])
                if not boxes_ner:
                    boxes_ner.append([np.sum(boxes1[:, 1]) / boxes1.shape[0], boxes2[i, 1], boxes2[i, 0],
                                      np.sum(boxes1[:, 3]) / boxes1.shape[0], boxes2[i, 3], boxes2[i, 2]])
                boxes_ner = np.array(boxes_ner)

                bexes_3d.append([boxes_ner[:, 0].sum() / boxes_ner.shape[0], boxes_ner[0, 1],
                                 boxes_ner[:, 2].sum() / boxes_ner.shape[0],
                                 boxes_ner[:, 3].sum() / boxes_ner.shape[0], boxes_ner[0, 4],
                                 boxes_ner[:, 5].sum() / boxes_ner.shape[0]])

    return np.array(bexes_3d, dtype=int)


def judge_bound_3d(pred_3ds, size):
    pred_cent = np.array((pred_3ds[:, :3] + pred_3ds[:, 3:]) / 2, dtype=np.uint)
    # 限制最小值
    pred_cent[:, 0] = np.clip(pred_cent[:, 0], 64, 9999)
    pred_cent[:, 1] = np.clip(pred_cent[:, 1], 64, 9999)
    pred_cent[:, 2] = np.clip(pred_cent[:, 2], 32, 9999)

    pred_3ds = np.hstack((pred_cent, pred_cent)) + np.array([-64, -64, -32, 64, 64, 32])
    new_pred_3ds=[]
    for pred_3d in pred_3ds:
        xmin,ymin,zmin,xmax,ymax,zmax = pred_3d
        if pred_3d[3] > size[1]:
            xmax = size[0]
            xmin = size[0]-128
        if pred_3d[4] > size[1]:
        ...
        if pred_3d[5] > size[2]:
            zmax = size[2]
            zmin = size[2] - 64
        new_pred_3ds.append([xmin, ymin, size[2]-zmax, xmax, ymax, size[2]-zmin])
    return new_pred_3ds


def judge_bound_2d(pred_boxs, size):
    new_boxs = []
    for pred_box in pred_boxs:
        x1, y1, x2, y2 = list(map(int, pred_box))
        w, h = size
        if x1 < 0:
            x2 = x2 - x1
            x1 = 0
